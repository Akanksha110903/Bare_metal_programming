...
void tim3_pc6_input_capture(void)
{
	...
	///SET PC6 to alternate fun mode
	GPIOC->MODER &= ~(1U<<12);
	GPIOC->MODER |= (1U<<13);

	//ste PC6 to alternate function type to TIM3 CH1  (AF2)
	GPIOC->AFR[0] &= ~(1U<<24);
	GPIOC->AFR[0] |=  (1U<<25);
	GPIOC->AFR[0] &= ~(1U<<26);
	GPIOC->AFR[0] &= ~(1U<<27);
	...
	//enable clock acces to timer 3
	RCC->APB1ENR |= TIM3EN;

	//set prescalar
	TIM3->PSC =16000 - 1; //16 000 000/1600 = 10 000 Hz
